...
contract SolidarySystemMetrics is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    // ...existing code...
...
function setSolidarityProjectWallet(address newWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function createSnapshot(string calldata extraNote) external onlyRole(SNAPSHOT_CREATOR_ROLE) returns (uint256) {
        // ensure interval
    require(block.timestamp >= lastSnapshotTimestamp + snapshotIntervalSeconds, "SolidarySystemMetrics: snapshot interval not passed");
    ...
    require(id > 0 && id <= _snapshotIdCounter.current(), "SolidarySystemMetrics: snapshot not found");
        return snapshots[id];
    }
    ...
    require(snapshotId > 0 && snapshotId <= _snapshotIdCounter.current(), "SolidarySystemMetrics: snapshot not found");
